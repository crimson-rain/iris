...
func update_total_loading_progress() -> void:
	var partial_total: Variant = _scene_loading_progress
...
func _show_all_draw_passes_once() -> void:
	var all_materials: Variant = _traverse_folders(_spatial_shader_material_dir)
	var total_material_count: Variant = all_materials.size()
	var cached_material_count: Variant = 0
	for material_path: Variant in all_materials:
...
func _traverse_folders(dir_path:String) -> PackedStringArray:
	...
	if not dir_path.ends_with("/"):
		dir_path += "/"
	var dir: Variant = DirAccess.open(dir_path)
	if not dir:
	...
	if dir.list_dir_begin() != OK:
		push_error("failed to access the path ", dir_path)
		return []
	var file_name: Variant = dir.get_next()
	while file_name != "":
		if not dir.current_is_dir():
			var matches : bool = false
			for extension: Variant in _matching_extensions:
			...
			var subfolder_name: Variant = file_name
...
func _load_material(path: String) -> void:
	var material_shower: Variant = MeshInstance3D.new()
	material_shower.mesh = _mesh
	var material: Variant = ResourceLoader.load(path) as Material
