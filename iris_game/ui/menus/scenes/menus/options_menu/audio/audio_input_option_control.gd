...
func _on_setting_changed(value: Variant) -> Variant:
	if value is int and value >= option_values.size():
		return null
	var device_name: String = option_values[value] as String
	if device_name:
		AudioServer.input_device = device_name
	return super._on_setting_changed(value)
